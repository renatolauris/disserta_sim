...
end.temp <- "C:/Users/renat/Dropbox/Dissertacao-Renato/2.simulacoes de causalidade/results"
setwd(end.temp)
...
alldata %>% 
  filter(!method %in% c("knn","knnbig"),measure=="mse",
         n==2000,d==4,sigma==3) %>% 
...
alldata %>% 
  filter(!method %in% c("knn","knnbig"),measure=="bias",
         n==2000,d==4,sigma==3) %>% 
  select(method,value,setup,n) %>% 
  mutate(value=round(value, 3),n=as.integer(n)) %>% 
  pivot_wider(names_from = method ,values_from = value
...
params.x1.setup = function(n=500,d=4,sigma=3,eta = 0.1,setup = "A") {
  ...
  results <- list(
    ...
    predtruth = list(tau = params_test$tau,cf = cf.pred$predictions,dml = dml.pred)
...
# 4B. true tau for X2 -----------------------------------------------------
tautruth2 <- lapply(setupvals, function(type) {
  tt <- params.x2.setup(n=500,d=4,sigma=3,setup = type)
  data1 <- data.frame(tt$params_test$X,
                      tau=tt$params_test$tau,setup = type)
  data1
})
tautruth3 <- do.call(rbind.data.frame, tautruth2)

ggplot(tautruth3)+
  geom_line(aes(x=X2, y=tau))+
  facet_wrap(~setup)+
...
ggplot(comparetauX12, aes(x=X1))+
  geom_line(aes(y=tau,color="Tau"))+
  geom_line(aes(y=cf,color="CF"))+
  geom_line(aes(y=dml,color="DML"))+
  facet_wrap(~setup)+
  labs(color = "",y = "tau",
